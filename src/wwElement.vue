...
export default {
  ...
  data() {
    return {
      // internal mirror for front runtime (non-editor)
  ...
  computed: {
    ...
    isOn: {
      get() {
        // In Editor: always reflect content (so it updates side panel + bindings)
        return this.isEditor ? !!this.content.checked : this.internalOn;
      },
      set(v) {
        if (this.isEditor) {
          // Update content in editor (with undo/redo support)
  ...
  watch: {
    'content.checked'(val) {
      // When bound/externally updated in front, keep internal mirror in sync
  ...
  methods: {
    onToggle() {
      ...
      this.isOn = !this.isOn;
      // Trigger workflows (provide both 'event' and 'payload' keys for compatibility)
      const data = { checked: this.isOn };
      this.$emit('trigger-event', { name: 'change', event: data, payload: data });
    },
    emitFocus() { this.$emit('trigger-event', { name: 'focus', event: {}, payload: {} }); },
    emitBlur() { this.$emit('trigger-event', { name: 'blur', event: {}, payload: {} }); }
  }
};
</script>
